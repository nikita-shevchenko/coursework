...
class Student(UserMixin, db.Model):
    ...
    def __init__(self, student_email, student_password, student_name, group_name):
...
class Lection(db.Model):
    ...
    lection_content = db.Column(db.Text)
    ...
    def __init__(self, lection_name, lection_author, lection_content):
...
class Label(db.Model):
    ...
    label_number = db.Column(db.Integer)
...
class Resource(db.Model):
    ...
    resource_source = db.Column(db.String(500))
    rating = db.Column(db.Integer)
    lection_name = db.Column(db.String(500), nullable=False)
    labels = db.relationship('Label', secondary=resources_have_labels_table)

    def __init__(self, resource_name, resource_content, resource_source, rating, lection_name):
...
class Test(db.Model):
    ...
    __table_name__ = (db.ForeignKeyConstraint(('variant', 'laboratory_theme'),
...
class Task(db.Model):
    variant = db.Column(db.Integer, primary_key=True)
    laboratory_theme = db.Column(db.String(500), primary_key=True)
    laboratory_task = db.Column(db.Text)
...
class Laboratory(db.Model):
    laboratory_theme = db.Column(db.String(500), primary_key=True)
    laboratory_number = db.Column(db.Integer)
...
class Implementation(db.Model):
    ...
    student_email = db.Column(db.String(500), primary_key=True)
    laboratory_theme = db.Column(db.String(500), nullable=False)
    status = db.Column(db.String(100))
    implementation_content = db.Column(db.Text)
